...
def get_nx_graph(file_path, full_node_list, sep='\t'):
    ...
    # graph = nx.read_edgelist('./data/delicious/0.input/delicious.txt', create_using=nx.DiGraph, delimiter=",", nodetype=int)
...
#    return F1
    ...
    for i in range(len(y_true)): 
        y_true_i = torch.Tensor(y_true[i])
        y_true_i = y_true_i.reshape(1, -1)

        pre = torch.sum(torch.multiply(y_pred, y_true_i))/(torch.sum(y_pred)+1E-9)
        rec = torch.sum(torch.multiply(y_pred, y_true_i))/(torch.sum(y_true_i)+1E-9)
        F1 = 2 * pre * rec / (pre + rec+1E-9)
      
        r.append(F1)
        p.append(pre)
        rc.append(rec)
    #print("recall: ", max(rc), "pre: ", max(p))
    #F1 = F1 / len(y_true)
...
#    return torch.Tensor(query), torch.Tensor(gt)
    ...
    for i in range(len(query)):
        file_gt = open(path + dataset + '/gt/' + dataset + f"{dd[0]}d-{i}.gt", 'r')
        result = []
        for line in file_gt:
            vec = [0 for i in range(vec_length)]
            line = line.strip()
    ...
    gt1 = []
    for i in range(len(query)):
        file_gt = open(path + dataset + '/gt/' + dataset + f"{dd[1]}d-{i}.gt", 'r')
        result = []
        for line in file_gt:
            vec = [0 for i in range(vec_length)]
            line = line.strip()
            ...
            result.append(vec)
        gt1.append(result)
    return torch.Tensor(query), gt, gt1
