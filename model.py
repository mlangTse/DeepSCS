...
class PretrainModel(nn.Module):
    def __init__(self, input_dim, config):
        ...
        self.encoder = LGCN(hops=config.hops, 
                        input_dim=input_dim, 
...
#        TotalLoss += torch.sum(torch.max(ones, logits_aa - logits_ba)**2, dim=-1)
#        TotalLoss += torch.sum(torch.max(ones, logits_bb - logits_ab)**2, dim=-1)
